...
noderob.prototype = {
    ...
    pollSendMotorCommands: function(){
        var self = this;
        self.m_pollSendMotorCommandss = setInterval(function () {
            self.log('x ' + joyX + ' y ' + joyY, 1, 'white');
            //log('x ' + joyX + ' y ' + joyY, 1);
            if (skip)
                return;
            self.controlDifferentialMotors(joyX, joyY);
        }, POLL_INTERVAL);
    },

    sendData: function(leftMotor, rightMotor, direction) {
        var self = this;
    ...
    driveMotors: function (leftMotor, rightMotor, direction) {
        var self = this;
        leftMotor = Math.abs(self.fixDec(leftMotor));
        rightMotor = Math.abs(self.fixDec(rightMotor));
        ...
        leftMotor = Math.round(self.perc(leftMotor, 255) * 100);
        rightMotor = Math.round(self.perc(rightMotor, 255) * 100);

        self.sendData(leftMotor, rightMotor, direction);
    },

    controlDifferentialMotors: function (x, y) {
        var self = this;
        ...
        if (moveY > 0) {
            ...
            if (x < THRESHOLD_LOW) {
                ...
                leftMotor = leftMotor - self.perc(leftMotor, reducePerc);
            }
            // right
            if (x > THRESHOLD_HIGH) {
                reduceX = X_CENTER - x;
                reducePerc = (reduceX / X_CENTER) * 100;
                rightMotor = rightMotor + self.perc(rightMotor, reducePerc);
            }
        }
        ...
        if (moveY < 0) {
            ...
            if (x < THRESHOLD_LOW) {
                reduceX = X_CENTER - x;
                reducePerc = (reduceX / X_CENTER) * 100;
                leftMotor = Math.abs(leftMotor) + self.perc(leftMotor, reducePerc);
            }
            /** RIGHT **/
            if (x > THRESHOLD_HIGH) {
                reduceX = X_CENTER - x;
                reducePerc = (reduceX / X_CENTER) * 100;
                rightMotor = Math.abs(rightMotor) - self.perc(rightMotor, reducePerc);
            }
        }
        self.driveMotors(leftMotor, rightMotor, direction);
    },
    ...
    initJoystick: function(){
        ...
        Joystick.create("/dev/input/js0", function (err, joystick) {
            ...
            joystick.on("button:lb:press", function () {
                self.driveMotors(1, 1, 'sharpLeft');
                skip = 1;
            });
            joystick.on("button:lb:release", function () {
                skip = 0;
                self.driveMotors(0, 0, 'fwd');
            });
            joystick.on("button:rb:press", function () {
                self.driveMotors(1, 1, 'sharpRight');
                skip = 1;
            });
