...
var SERVO_CENTER_HAT_MOTORS = 50; // reverse 25 100% | center 50 0% | forward 75 100%
var SERVO_CENTER_HAT = 50; // 0 degrees = 0 | 90 degrees (center) = 50 | 180 degrees = 100
    ...
    switch (SERVO_MODE) {
        ...
        case 'ADAHAT':
        {
            ...
            if (self.m_leftMotor >= 0 && self.m_rightMotor >= 0) {
                self.m_leftMotor = (self.m_leftMotor * 0.25) + SERVO_CENTER_HAT_MOTORS;
                self.m_rightMotor = (self.m_rightMotor * 0.25) + SERVO_CENTER_HAT_MOTORS;
            }

            /** moving back **/
            if (self.m_leftMotor <= 0 && self.m_rightMotor <= 0) {
                self.m_leftMotor = SERVO_CENTER_HAT_MOTORS + self.m_leftMotor * 0.25;
                self.m_rightMotor = SERVO_CENTER_HAT_MOTORS + self.m_rightMotor * 0.25;
            }
        ...
        case 'SERVOBLASTER':
        {
            ...
            piblaster.setPwm(22, self.m_rightMotor);
        }
    }
};

NodeRob.prototype._straightMotors = function () {
    var self = this;
    self.m_leftMotor = self.m_rightMotor = joyY;
...
NodeRob.prototype.initJoystick = function (i_callBack) {
    ...
    Joystick.create("/dev/input/js0", function (err, joystick) {
         ...
         *****************************/
        var through = 0.16; // 0.50 is full through from side to side which is too much for steering
        joystick.on("stick:2:horizontal:right", function (position) {
            self.m_servo0 =  (position * through) + SERVO_CENTER_HAT;
        });
        joystick.on("stick:2:horizontal:left", function (position) {
            self.m_servo0 =  SERVO_CENTER_HAT - (position * through);
        });
        ...
        joystick.on("stick:2:vertical:down", function (position) {
            /*self.m_servo1 = SERVO_CENTER_HAT_MOTORS - Math.round(position / 10);
             if (self.m_servo1 < 1)
             self.m_servo1 = 0;
...
NodeRob.prototype.initServos = function () {
    var self = this;
    for (var i = 0; i < TOTAL_SERVOS; i++) {
        var s = 'self.m_servo' + i + ' = SERVO_CENTER_HAT_MOTORS';
        eval(s);
    }
};
...
NodeRob.prototype.start = function () {
    ...
    self.m_pollinghandle = setInterval(function () {
        ...
        /** Differential motors - tank **/
        // self._differentialMotors();

        /** Straight motors + steering - rc car **/
        self._straightMotors();
